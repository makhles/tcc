...
Não obstante, o sistema atual está carente de algumas funcionalidades consideradas primordiais, descritas nos itens a seguir. A implementação de tais funcionalidades no sistema atual é complexa, pois a sua estrutura não foi projetada pensando em tais funcionalidades. Adicionalmente, as bibliotecas e tecnologias utilizadas no sistema atual são antigas\footnote{Algumas estão, inclusive, descontinuadas.}, dificultando seu desenvolvimento e sua manutenibilidade. Justifica-se, portanto, a atualização tecnológica do sistema de inscrição.
...
%Diz-se que a entidade \texttt{PoloCursoNivel} é a entidade fonte e a entidade \texttt{Polo} é a entidade alvo da associação.
...
O diagrama de classes do sistema foi dividido em duas imagens devido ao tamanho destas. A Figura~\ref{fig:uml_classes1} mostra as classes envolvidas na obtenção dos programas disponíveis.
...
\begin{figure}[!ht]
  \caption{\label{fig:uml_classes1} Modelo entidade-relacionamento relacionado às inscrições. }
  \begin{center}
    \includegraphics[width=1.0\textwidth]{/uml/inscricoes.png}
  \end{center}
  \fonte{Autor.}
\end{figure}
