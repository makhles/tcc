...
Os requisitos funcionais descrevem os serviços que o sistema deve fornecer, como deve reagir a entradas específicas e como deve se comportar em determinadas situações. Visando o entendimento por todas as partes interessadas, a especificação dos requisitos é feita aqui de forma abstrata (em alto nível).
...
\begin{figure}[!ht]
  ...
  \begin{center}
    \includegraphics[width=1.0\textwidth]{/use_cases.png}
  \end{center}
...
\begin{table}[!htb]
  \IBGEtab{%
    ...
    \begin{tabular}{ll}
      ...
      \midrule \midrule
      Ver programas disponíveis & \texttt{programas.xhtml} \\ \hline 
      Registrar                 & \multirow{3}{*}{\texttt{login.xhtml}} \\
      Fazer \emph{login}        & \\
      Fazer \emph{logout}       & \\
      \midrule
      Criar inscrição           & \multirow{3}{*}{\texttt{inscricoes.xhtml}} \\
      Editar inscrição          & \\
      Excluir inscrição         & \\
      \midrule
...
% ---
Como pode ser visto no diagrama de casos de uso, Figura~\ref{fig:use_cases}, o cadastro e a autenticação no sistema são feitos através do Sistema de Autenticação Centralizada (CAS), que permite a navegação entre os diversos sistemas da UFSC com apenas uma autenticação. Ao clicar nos links de acesso (vide destaque na Figura~\ref{fig:acesso_sistema}), o candidato é automaticamente transferido à página do CAS (vide Figura~\ref{fig:autenticacao}), onde pode cadastrar-se e autenticar-se perante o sistema. Após autenticado, o candidato é automaticamente transferido para a página de inscrições.
...
Salienta-se que o CAS é um sistema já existente e utilizado por diversos outros sistemas da UFSC, e que o CAPG Inscrição atua apenas como cliente.
...
% ---
\subsection{Salvamento e finalização de uma inscrição}\label{sec:envio_documentos}
% ---
