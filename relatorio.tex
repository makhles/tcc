...
% ---
Um requisito não-funcional que não havia sido considerado no início deste projeto e que surgiu relativamente próximo ao seu fim foi a criação de uma interface que pudesse ser utilizada por dispositivos móveis de forma responsiva (ou fluida). As imagens utilizadas na exemplificação das funcionalidades mostradas a seguir foram feitas com a versão \emph{desktop} do navegador. A versão \emph{mobile} das imagens pode ser vista no Anexo~\ref{anexo:mobile}.
